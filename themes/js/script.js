...
function setColor(color) {
	...
	localStorage.setItem(`color`, bgc);
...
function addNote() {
	...
	if(value.trim() !== '') {
		...
		const elem = $(`
			<div class="box" id="${numberOfNotes}" style="background-color: ${notes[numberOfNotes].backgroundColor};">
			<div class="nav">
			<div>
...
window.addEventListener(`load`, function() {
	...
	document.querySelector(`.section__input__text`).style.backgroundColor = bgcFromLocalStorage;
});
...
if(newObjectFromLocalStorage !== null) {
	...
	for(let i = 0; i <= newObjectFromLocalStorage.length - 1; i++) {
		let getElementFromLocalStorage = $(`
			<div class="box" style="background-color: ${notes[i].backgroundColor};">
			<div class="nav">
			<div>
			<svg class="star" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
			fill="currentColor">
